...
function LiveAssetCard({ asset, paymentReady, paymentDecimals }: { asset: InvestorCatalogAsset; paymentReady: boolean; paymentDecimals: number }) {
  ...
  return <><Card><CardHeader><div className="flex items-start justify-between gap-3"><div><Badge variant="secondary">{asset.asset_type}</Badge><CardTitle className="mt-3">{asset.name}</CardTitle><CardDescription>{asset.ticker} · {asset.network} · settles in {asset.settlement_currency}</CardDescription></div><Badge variant={ready ? "success" : "warning"}>{ready ? "Ready for quote" : "Action required"}</Badge></div></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">{asset.description}</p><div className="grid gap-3 text-sm sm:grid-cols-2"><Value label="Issuer inventory" value={`${Number(asset.issuer_inventory).toLocaleString()} units`} /><Value label="Your equity balance" value={`${Number(asset.investor_balance).toLocaleString()} units`} /><Value label="Equity mint" value={asset.mint_address} mono /><Value label="Your equity token account" value={asset.investor_token_account} mono /></div><div className="flex items-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="size-4 text-primary" />{asset.disclosure}</div><Button className="w-full" disabled={!ready} onClick={() => setQuoteOpen(true)}>Buy {asset.ticker}</Button>{!asset.investor_account_ready && <p className="text-xs text-amber-700 dark:text-amber-300">Your equity token account is not active yet.</p>}{!paymentReady && <p className="text-xs text-amber-700 dark:text-amber-300">Create and fund your cNGN account before requesting a quote.</p>}</CardContent></Card><InvestorQuoteDialog asset={asset} open={quoteOpen} onOpenChange={setQuoteOpen} paymentDecimals={paymentDecimals} /></>;
}
