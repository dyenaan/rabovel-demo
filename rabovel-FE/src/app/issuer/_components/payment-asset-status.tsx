...
export function PaymentAssetStatusCard() {
  ...
  async function createAccount() {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Could not create the broker cNGN account.");
    } finally {
      setIsCreating(false);
    }
  }
  ...
  return <Card className="md:col-span-2"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>CNGN payment rail</CardTitle><CardDescription className="mt-1">Fund the Admin wallet with test SOL, create the broker’s associated cNGN account, then fund that token-account address from the faucet.</CardDescription></div><div className="flex flex-wrap items-center gap-2"><Badge variant={status?.ready ? "success" : "warning"}>{status?.ready ? "Ready" : "Not ready"}</Badge>{status && !status.broker_account_verified && <Button type="button" size="sm" disabled={isCreating || isLoading || !status.mint_verified} onClick={() => void createAccount()}>{isCreating && <RefreshCw className="animate-spin" />}{isCreating ? "Creating…" : "Create broker cNGN account"}</Button>}<Button type="button" variant="outline" size="sm" disabled={isLoading || isCreating || !token} onClick={() => void loadStatus()}><RefreshCw className={isLoading ? "animate-spin" : undefined} />Refresh</Button></div></div></CardHeader><CardContent className="grid gap-4 text-sm sm:grid-cols-2"><Detail label="Network" value={status?.network ?? (isLoading ? "Loading…" : "Unavailable")} /><Detail label="Token program" value={status?.token_program ?? "Unverified"} /><Detail label="Decimals" value={status?.decimals?.toString() ?? "Unverified"} /><Detail label="Broker cNGN balance (base units)" value={status?.broker_balance_base_units ?? "Unverified"} /><AddressDetail label="Admin fee-payer wallet" address={status?.fee_payer_address} hint="Send test SOL here. The backend uses it to pay transaction and account-creation fees." /><AddressDetail label="Broker wallet (owner)" address={status?.broker_owner_address} hint="This is the Solana keypair’s public address and owns the token account below." /><AddressDetail label="Broker cNGN token account" address={status?.broker_token_account} hint="Use this address as the faucet recipient once the account is created." /><AddressDetail label="cNGN mint" address={status?.mint_address} />{error && <p role="alert" className="sm:col-span-2 text-destructive">{error}</p>}{status?.error && <p className="sm:col-span-2 text-amber-700 dark:text-amber-300">{status.error}</p>}</CardContent></Card>;
}
